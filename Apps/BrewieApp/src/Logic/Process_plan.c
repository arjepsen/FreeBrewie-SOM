...
static void process_plan_append_mash_steps(const recipe_draft_t *draft, process_plan_t *plan);
static void process_plan_append_sparge_step(const recipe_draft_t *draft, process_plan_t *plan);
static void process_plan_append_boil_steps(const recipe_draft_t *draft, process_plan_t *plan);
static void process_plan_append_cooling_step(const recipe_draft_t *draft, process_plan_t *plan);
static void process_plan_append_fermentation_steps(const recipe_draft_t *draft,
                                                   process_plan_t *plan);
...
static void process_plan_append_mash_steps(const recipe_draft_t *draft, process_plan_t *plan)
{
    ...
    (void)process_plan_append(plan,
                              &(process_plan_step_t){
                                  ...
                                  .label = "Mash in",
                                  .target_temperature_c = draft->brewing.mash_in_temperature_c,
                                  .water_amount_dl = draft->brewing.mash_in_water_dl,
                                  .duration_min = 0U,
                                  .boil_elapsed_min = 0U,
                                  .source_index = 0U});

    for (index = 0U;
         index < draft->brewing.mash_step_count && index < RECIPE_DRAFT_MAX_MASH_STEPS;
         ++index)
    {
        (void)process_plan_append(plan,
                                  &(process_plan_step_t){
                                      ...
                                      .target_temperature_c =
                                          draft->brewing.mash_steps[index].temperature_c,
                                      .water_amount_dl = 0U,
                                      .duration_min = draft->brewing.mash_steps[index].time_min,
                                      .boil_elapsed_min = 0U,
                                      .source_index = index});
    }
}
...
static void process_plan_append_sparge_step(const recipe_draft_t *draft, process_plan_t *plan)
{
    if (draft->brewing.sparge_water_dl == 0U &&
    ...
    (void)process_plan_append(plan,
                              &(process_plan_step_t){
                                  ...
                                  .label = "Sparge",
                                  .target_temperature_c = draft->brewing.sparge_temperature_c,
                                  .water_amount_dl = draft->brewing.sparge_water_dl,
                                  .duration_min = draft->brewing.sparge_time_min,
                                  .boil_elapsed_min = 0U,
                                  .source_index = 0U});
}
...
static void process_plan_append_boil_steps(const recipe_draft_t *draft, process_plan_t *plan)
{
    ...
    (void)process_plan_append(plan,
                              &(process_plan_step_t){
                                  ...
                                  .water_amount_dl = 0U,
                                  .duration_min = draft->brewing.boil_time_min,
                                  .boil_elapsed_min = 0U,
                                  .source_index = 0U});
    ...
    for (index = 0U; index < draft->hop_count && index < RECIPE_DRAFT_MAX_HOPS; ++index)
    {
        ...
        boil_elapsed_min = 0U;
        if (draft->hops[index].boil_time_min < draft->brewing.boil_time_min)
        {
            boil_elapsed_min =
                (uint16_t)(draft->brewing.boil_time_min - draft->hops[index].boil_time_min);
        }

        (void)process_plan_append(plan,
                                  &(process_plan_step_t){
                                      .kind = PROCESS_PLAN_STEP_HOP_ADDITION,
                                      .label = draft->hops[index].name,
...
static void process_plan_append_cooling_step(const recipe_draft_t *draft, process_plan_t *plan)
{
    (void)process_plan_append(plan,
                              &(process_plan_step_t){
                                  ...
                                  .label = "Cool",
                                  .target_temperature_c = draft->brewing.cooling_target_c,
                                  .water_amount_dl = 0U,
                                  .duration_min = 0U,
                                  .boil_elapsed_min = 0U,
                                  .source_index = 0U});
         ...
         ++index)
    {
        (void)process_plan_append(plan,
                                  &(process_plan_step_t){
                                      ...
                                      .label = draft->fermentation.steps[index].name,
                                      .target_temperature_c =
                                          draft->fermentation.steps[index].temperature_c,
                                      .water_amount_dl = 0U,
                                      .duration_min =
                                          (uint16_t)(draft->fermentation.steps[index].duration_days *
                                                     24U * 60U),
                                      .boil_elapsed_min = 0U,
                                      .source_index = index});
    }
...
/****************************************************************************************
 * @brief Build the first ordered process plan from the RAM-only recipe draft.
 *
...
bool process_plan_build_from_draft(const recipe_draft_t *draft, process_plan_t *plan)
{
    ...
    {
        plan->status_text = validation.status_text;
        return false;
    }

    process_plan_append_mash_steps(draft, plan);
    process_plan_append_sparge_step(draft, plan);
    process_plan_append_boil_steps(draft, plan);
    process_plan_append_cooling_step(draft, plan);
    process_plan_append_fermentation_steps(draft, plan);
    (void)process_plan_append(plan,
                              &(process_plan_step_t){
